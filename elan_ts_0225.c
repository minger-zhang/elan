...
static DEVICE_ATTR(update_fw_system, 0644, NULL, store_update_fw_from_system);
...
static DEVICE_ATTR(update_fw_sdcard, 0644, NULL, store_update_fw_from_sdcard);
...
static int __fw_packet_handler(struct i2c_client *client)
{
    ...
    uint8_t cmd_osr[37]      =   {0x04, 0x00, 0x23, 0x00, 0x03, 0x00, 0x04, 0x53, 0xD6, 0x00, 0x01};/* Get OSR*/
   ...
   //TP OSR
   rc = elan_ktf_ts_get_data(client,cmd_osr,buf_recv,37,67);
   if(rc < 0)
   {
        printk("Get TP OSR error\n");
	return rc;
   }
   elan_info("[elan] %s:(TP OSR) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n",__func__,buf_recv[0],buf_recv[1],buf_recv[2],buf_recv[3], \
  								   buf_recv[4],buf_recv[5],buf_recv[6],buf_recv[7]);
   osr =  buf_recv[7];
   printk("TP OSR = %d\n",osr);
    //TP RESOLUTION
    ...
    elan_info("[elan] %s: (Tp Resolution) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3] ,\
     ...
     if((((ts->fw_ver)&&0xff00)>>8) == 0x56){	
      ...
      elan_info("[elan] There is a chip\n");
    }else{
    	x = buf_recv[6] + buf_recv[10];
    	y = buf_recv[7] + buf_recv[11];
    	elan_info("[elan] There are two chip\n");
    }
    ts->x_resolution=(x-1)*osr;
    ts->y_resolution=(y-1)*osr; 
